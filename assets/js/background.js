...
function initStarField() {
  ...
  for (let layer = 1; layer <= 5; layer++) {
    const count = Math.floor(500 / layer); // Aumentado para mais estrelas
...
function createSpacecraft() {
  ...
  if (Math.random() < 0.0003) {
    ...
    spacecrafts.push({
      ...
      type: type,
      size: Math.random() * 3 + 2, // Bem pequeno
...
function createISS() {
  ...
  if (Math.random() < 0.00008 && !iss) {
    ...
    iss = {
      ...
      size: 6, // Maior que espaçonaves normais
      angle: Math.atan2(dy, dx),
      rotation: 0,
      lights: [
      ...
      fadePhase: 'fadein',
      fadeTimer: 0,
      fadeDuration: 180, // Fade muito longo
...
function checkCollisions() {
  ...
  for (let i = comets.length - 1; i >= 0; i--) {
    ...
    if (Math.random() > 0.05) continue; // Apenas 5% de chance
  ...
  for (let i = supernovas.length - 1; i >= 0; i--) {
    ...
    switch(supernova.phase) {
      case 'expand':
        ...
        supernova.opacity = Math.min(supernova.maxOpacity, supernova.radius / 50);
    ...
    if (supernova.phase !== 'contract' || supernova.opacity > 0.1) {
      ...
      ctx.arc(supernova.x, supernova.y, supernova.radius * 0.1, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    }
  }
